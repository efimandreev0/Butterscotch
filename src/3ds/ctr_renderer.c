...
static uint8_t *ctrReadPngBlobFromFile(FILE *fp, uint32_t blobOffset, uint32_t blobSize) {
    ...
    if (!buf) return NULL;
    if (fread(buf, 1, blobSize, fp) != blobSize) {
        free(buf);
        return NULL;
    }
    return buf;
...
static void ctrPushQuad(CtrRenderer *gl, GLuint textureId, float x0, float y0, float x1, float y1, float x2, float y2,
                        float x3, float y3, float u0, float v0, float u1, float v1, uint8_t r, uint8_t g, uint8_t b,
                        uint8_t a) {
    ctrPushQuadGradient(gl, textureId, x0, y0, x1, y1, x2, y2, x3, y3, u0, v0, u1, v1, r, g, b, a, r, g, b, a, r, g, b,
                        a, r, g, b, a);
}

static void ctrPushTriangleGradient(CtrRenderer *gl, float x1, float y1, float x2, float y2, float x3, float y3,
                                    uint8_t r1, uint8_t g1, uint8_t b1, uint8_t a1, uint8_t r2, uint8_t g2, uint8_t b2,
                                    uint8_t a2, uint8_t r3, uint8_t g3, uint8_t b3, uint8_t a3) {
    ctrPushQuadGradient(gl, gl->whiteTexture, x1, y1, x2, y2, x3, y3, x3, y3, 0.5f, 0.5f, 0.5f, 0.5f, r1, g1, b1, a1,
                        r2, g2, b2, a2, r3, g3, b3, a3, r3, g3, b3, a3);
}

static void ctrPushTriangleSolid(CtrRenderer *gl, float x1, float y1, float x2, float y2, float x3, float y3, uint8_t r,
                                 uint8_t g, uint8_t b, uint8_t a) {
    ctrPushTriangleGradient(gl, x1, y1, x2, y2, x3, y3, r, g, b, a, r, g, b, a, r, g, b, a);
}

static void ctrAppendArcPoints(float *outX, float *outY, int32_t *count, float cx, float cy, float rx, float ry,
...
static void ctrBeginView(Renderer *renderer, int32_t viewX, int32_t viewY, int32_t viewW, int32_t viewH, int32_t portX,
    ...
    Matrix4f_ortho(&projection, (float) viewX, (float) (viewX + viewW), (float) (viewY + viewH), (float) viewY, -1.0f,
                   1.0f);
...
static void ctrBeginGUI(Renderer *renderer, int32_t guiW, int32_t guiH, int32_t portX, int32_t portY, int32_t portW,
                        int32_t portH) {
    CtrRenderer *gl = (CtrRenderer *) renderer;
    ctrFlushBatch(gl);
    glBindTexture(GL_TEXTURE_2D, 0);
...
static void extract_tpag_from_ram(CtrRenderer *gl, DataWin *dw, uint32_t tId, uint16_t *atlas_pixels, int atlas_w,
                                  int atlas_h) {
    ...
    int extX = item->sourceX;
    int extY = item->sourceY;
    ...
    for (int cy = 0; cy < tpag->chunksY; cy++) {
        for (int cx = 0; cx < tpag->chunksX; cx++) {
            ...
            chunk->srcX = cx * 1024;
            chunk->srcY = cy * 1024;
            chunk->width = extW - chunk->srcX;
            if (chunk->width > 1024) chunk->width = 1024;
            chunk->height = extH - chunk->srcY;
            if (chunk->height > 1024) chunk->height = 1024;
...
static void extract_tpag_direct_from_file(CtrRenderer *gl, DataWin *dw, uint32_t tId, FILE *f, int atlas_w,
                                          int atlas_h) {
    TexturePageItem *item = &dw->tpag.items[tId];
    int extW = item->sourceWidth > 0 ? item->sourceWidth : 1;
    int extH = item->sourceHeight > 0 ? item->sourceHeight : 1;
...
static void ctrOnRoomChanged(Renderer *renderer, int32_t roomIndex) {
    ...
    for (int pid = 0; pid < 256; pid++) {
        ...
        int pendingCount = 0;
        for (uint32_t i = 0; i < gl->tpagCount; i++) {
            if (dw->tpag.items[i].texturePageId == pid && gl->tpags[i].keepResident && !gl->tpags[i].isLoaded)
                pendingCount++;
        }
        ...
        if (fread(&hdr, sizeof(hdr), 1, f) == 1 && hdr.magic == ATLAS_MAGIC) {
            ...
            if (pendingCount >= 2) {
                ...
                if (atlas_pixels) {
                    if (fread(atlas_pixels, 1, data_size, f) == data_size) {
                        for (uint32_t tId = 0; tId < gl->tpagCount; tId++) {
                            if (dw->tpag.items[tId].texturePageId == pid && gl->tpags[tId].keepResident && !gl->tpags[
                                    tId].isLoaded) {
                                extract_tpag_from_ram(gl, dw, tId, atlas_pixels, hdr.width, hdr.height);
                            }
                        }
            ...
            if (!batchLoaded) {
                for (uint32_t tId = 0; tId < gl->tpagCount; tId++) {
                    if (dw->tpag.items[tId].texturePageId == pid && gl->tpags[tId].keepResident && !gl->tpags[tId].
                        isLoaded) {
...
static void lerp2D_quad(float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3, float tx,
                        float ty, float *outX, float *outY) {
...
static void ctrDrawSprite(Renderer *renderer, int32_t tpagIndex, float x, float y, float originX, float originY,
    ...
    if (angleDeg == 0.0f) {
        ...
        if (singleChunk) {
            ...
            ctrDrawTpagRegion(gl, tpagIndex, 0.0f, 0.0f, (float) tpag->sourceWidth, (float) tpag->sourceHeight, x0, y0,
                              x1, y0, x1, y1, x0, y1, r, g, b, a);
        }
    } else {
...
static void ctrDrawSpritePart(Renderer *renderer, int32_t tpagIndex, int32_t srcOffX, int32_t srcOffY, int32_t srcW,
...
static void ctrDrawSpritePos(Renderer *renderer, int32_t tpagIndex, float x1, float y1, float x2, float y2, float x3,
...
static void ctrDrawTile(Renderer *renderer, RoomTile *tile, float offsetX, float offsetY) {
    ...
    renderer->vtable->drawSpritePart(renderer, tpagIndex, atlasOffX, atlasOffY, srcW, srcH, drawX, drawY, tile->scaleX,
                                     tile->scaleY, color, alpha);
}

static void ctrDrawTiled(Renderer *renderer, int32_t tpagIndex, float originX, float originY, float x, float y,
    ...
    for (int32_t iy = 0; iy < countY; iy++) {
        ...
        for (int32_t ix = 0; ix < countX; ix++) {
            ...
            renderer->vtable->drawSprite(renderer, tpagIndex, drawX, drawY, originX, originY, xscale, yscale, 0.0f,
                                         color, alpha);
        }
    }
}

static void emitColoredQuad(CtrRenderer *gl, float x0, float y0, float x1, float y1, float r, float g, float b,
                            float a) {
    uint8_t ca = ctrAlphaToByte(a);
    if (ca == 0) return;
    ctrPushQuad(gl, gl->whiteTexture, x0, y0, x1, y0, x1, y1, x0, y1, 0.5f, 0.5f, 0.5f, 0.5f, (uint8_t) (r * 255.0f),
                (uint8_t) (g * 255.0f), (uint8_t) (b * 255.0f), ca);
}

static void ctrDrawRectangle(Renderer *renderer, float x1, float y1, float x2, float y2, uint32_t color, float alpha,
                             bool outline) {
    CtrRenderer *gl = (CtrRenderer *) renderer;
...
static void ctrDrawLine(Renderer *renderer, float x1, float y1, float x2, float y2, float width, uint32_t color,
                        float alpha) {
    CtrRenderer *gl = (CtrRenderer *) renderer;
    ...
    if (fabsf(x1 - x2) < 0.01f || fabsf(y1 - y2) < 0.01f) {
        ...
        ctrPushQuad(gl, gl->whiteTexture, rx1, ry1, rx2, ry1, rx2, ry2, rx1, ry2, 0.5f, 0.5f, 0.5f, 0.5f, cr, cg, cb,
                    ca);
        return;
    }
    float dx = x2 - x1;
    ...
    ctrPushQuad(gl, gl->whiteTexture, x1 + px, y1 + py, x1 - px, y1 - py, x2 - px, y2 - py, x2 + px, y2 + py, 0.5f,
                0.5f, 0.5f, 0.5f, cr, cg, cb, ca);
}

static void ctrDrawLineColor(Renderer *renderer, float x1, float y1, float x2, float y2, float width, uint32_t color1,
                             uint32_t color2, float alpha) {
    CtrRenderer *gl = (CtrRenderer *) renderer;
    uint8_t c1r = BGR_R(color1);
    uint8_t c1g = BGR_G(color1);
    uint8_t c1b = BGR_B(color1);
    uint8_t c2r = BGR_R(color2);
    uint8_t c2g = BGR_G(color2);
    uint8_t c2b = BGR_B(color2);
    uint8_t ca = ctrAlphaToByte(alpha);
    float actualWidth = (width < 2.0f) ? 2.0f : width;
    ...
    float px = (-dy / len) * halfW;
    float py = (dx / len) * halfW;
    ctrPushQuadGradient(gl, gl->whiteTexture, x1 + px, y1 + py, x1 - px, y1 - py, x2 - px, y2 - py, x2 + px, y2 + py,
                        0.5f, 0.5f, 0.5f, 0.5f, c1r, c1g, c1b, ca, c1r, c1g, c1b, ca, c2r, c2g, c2b, ca, c2r, c2g, c2b,
                        ca);
}

static void ctrDrawTriangle(Renderer *renderer, float x1, float y1, float x2, float y2, float x3, float y3,
                            bool outline) {
    CtrRenderer *gl = (CtrRenderer *) renderer;
    if (outline) {
        ...
        ctrPushQuad(gl, gl->whiteTexture, x1, y1, x2, y2, x3, y3, x3, y3, 0.5f, 0.5f, 0.5f, 0.5f,
                    BGR_R(renderer->drawColor), BGR_G(renderer->drawColor), BGR_B(renderer->drawColor),
                    ctrAlphaToByte(renderer->drawAlpha));
    }
}
...
static void ctrDrawEllipse(Renderer *renderer, float cx, float cy, float rx, float ry, uint32_t color, float alpha,
...
static void ctrDrawCircle(Renderer *renderer, float x, float y, float radius, uint32_t color, float alpha, bool outline,
                          int32_t precision) {
    ctrDrawEllipse(renderer, x, y, fabsf(radius), fabsf(radius), color, alpha, outline, precision);
}

static void ctrDrawRoundrect(Renderer *renderer, float x1, float y1, float x2, float y2, float radx, float rady,
    ...
    if (width <= 0.0f || height <= 0.0f || radx <= 0.0f || rady <= 0.0f) {
    ...
    float pX[CTR_MAX_ROUNDRECT_POINTS];
    float pY[CTR_MAX_ROUNDRECT_POINTS];
    int32_t count = 0;
    ...
    ctrAppendArcPoints(pX, pY, &count, left + radx, top + rady, radx, rady, (float) M_PI, (float) M_PI + halfPi,
                       arcSegments, true);
...
static bool ctrResolveGlyph(CtrRenderer *gl, DataWin *dw, CtrFontState *state, FontGlyph *glyph, float cursorX,
                            float cursorY, uint32_t *outTpagIndex, float *outSrcX, float *outSrcY, float *outSrcW,
                            float *outSrcH, float *outLocalX0, float *outLocalY0) {
...
static void ctrDrawText(Renderer *renderer, const char *text, float x, float y, float xscale, float yscale,
                        float angleDeg) {
    CtrRenderer *gl = (CtrRenderer *) renderer;
    DataWin *dw = renderer->dataWin;
    ...
    uint8_t a = ctrAlphaToByte(renderer->drawAlpha);
    if (a == 0) return;
    ...
    for (int32_t lineIdx = 0; lineCount > lineIdx; lineIdx++) {
        ...
        while (lineLen > pos) {
            ...
            uint32_t tpagIndex;
            if (!ctrResolveGlyph(gl, dw, &fontState, glyph, cursorX, cursorY, &tpagIndex, &srcX, &srcY, &srcW, &srcH,
                                 &localX0, &localY0)) {
                cursorX += glyph->shift;
                continue;
            }
...
static void ctrDrawTextColor(Renderer *renderer, const char *text, float x, float y, float xscale, float yscale,
                             float angleDeg, int32_t _c1, int32_t _c2, int32_t _c3, int32_t _c4, float floatAlpha) {
...
static int32_t ctrCreateSpriteFromSurface(Renderer *renderer, int32_t x, int32_t y, int32_t w, int32_t h,
                                          bool removeback, bool smooth, int32_t xorig, int32_t yorig) { return -1; }
