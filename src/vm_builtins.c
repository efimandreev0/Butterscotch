...
int16_t VMBuiltins_resolveBuiltinVarId(const char *name) {
    ...
    BuiltinVarEntry *hit = (BuiltinVarEntry *) bsearch(name, BUILTIN_VAR_TABLE, count, sizeof(BuiltinVarEntry),
                                                       compareBuiltinVarEntry);
...
void VMBuiltins_checkIfBuiltinVarTableIsSorted(void) {
    ...
    for (size_t i = 1; count > i; i++) {
        ...
        requireMessageFormatted(
            cmp < 0,
            "BUILTIN_VAR_TABLE not strictly sorted at index %zu: '%s' vs '%s' (cmp=%d). Re-sort (LC_ALL=C) or remove duplicates!",
...
RValue VMBuiltins_getVariable(VMContext *ctx, int16_t builtinVarId, const char *name, int32_t arrayIndex) {
    ...
    switch (builtinVarId) {
        ...
        case BUILTIN_VAR_SPRITE_WIDTH: {
            ...
            if (inst->spriteIndex >= 0 && runner->dataWin->sprt.count > (uint32_t) inst->spriteIndex) {
                return RValue_makeReal(
                    (GMLReal) runner->dataWin->sprt.sprites[inst->spriteIndex].width * inst->imageXscale);
            }
            return RValue_makeReal(0.0);
        }
        case BUILTIN_VAR_SPRITE_HEIGHT: {
            ...
            if (inst->spriteIndex >= 0 && runner->dataWin->sprt.count > (uint32_t) inst->spriteIndex) {
                return RValue_makeReal(
                    (GMLReal) runner->dataWin->sprt.sprites[inst->spriteIndex].height * inst->imageYscale);
            }
            return RValue_makeReal(0.0);
        }
        case BUILTIN_VAR_SPRITE_XOFFSET: {
            ...
            if (inst->spriteIndex >= 0 && runner->dataWin->sprt.count > (uint32_t) inst->spriteIndex) {
                return RValue_makeReal(
                    (GMLReal) runner->dataWin->sprt.sprites[inst->spriteIndex].originX * inst->imageXscale);
            }
            return RValue_makeReal(0.0);
        }
        case BUILTIN_VAR_SPRITE_YOFFSET: {
            ...
            if (inst->spriteIndex >= 0 && runner->dataWin->sprt.count > (uint32_t) inst->spriteIndex) {
                return RValue_makeReal(
                    (GMLReal) runner->dataWin->sprt.sprites[inst->spriteIndex].originY * inst->imageYscale);
            }
            return RValue_makeReal(0.0);
        }
    ...
    switch (builtinVarId) {
        ...
        case BUILTIN_VAR_VIEW_ANGLE:
            if (arrayIndex >= 0 && MAX_VIEWS > arrayIndex)
                runner->views[arrayIndex].viewAngle = (float) RValue_toReal(val);
            return;
        case BUILTIN_VAR_VIEW_HBORDER:
...
static RValue builtinIsReal(MAYBE_UNUSED VMContext *ctx, RValue *args, int32_t argCount) {
    if (1 > argCount) return RValue_makeBool(false);
    bool result = args[0].type == RVALUE_REAL || args[0].type == RVALUE_INT32 || args[0].type == RVALUE_INT64 || args[0]
                  .type == RVALUE_BOOL;
...
static RValue builtinMpLinearStepCommon(VMContext *ctx, GMLReal goalX, GMLReal goalY, GMLReal stepsize,
                                        int32_t objIndex, bool checkall) {
    Runner *runner = (Runner *) ctx->runner;
    Instance *inst = (Instance *) ctx->currentInstance;
...
static RValue builtinMpPotentialStepCommon(VMContext *ctx, GMLReal goalX, GMLReal goalY, GMLReal stepsize,
                                           int32_t objIndex, bool checkall) {
    Runner *runner = (Runner *) ctx->runner;
    Instance *inst = (Instance *) ctx->currentInstance;
    if (inst == nullptr) return RValue_makeBool(false);
...
static RValue builtinIniOpen(VMContext *ctx, RValue *args, int32_t argCount) {
    ...
    if (runner->currentIni != nullptr && runner->currentIniPath != nullptr && strcmp(runner->currentIniPath, path) ==
        0) {
        return RValue_makeUndefined();
    }
...
static RValue builtin_drawSpriteTiled(VMContext *ctx, RValue *args, MAYBE_UNUSED int32_t argCount) {
    ...
    Renderer_drawSpriteTiled(runner->renderer, spriteIndex, subimg, x, y, 1.0f, 1.0f, roomW, roomH, 0xFFFFFF,
...
static RValue builtin_drawSpritePartExt(VMContext *ctx, RValue *args, MAYBE_UNUSED int32_t argCount) {
    ...
    Renderer_drawSpritePartExt(runner->renderer, spriteIndex, subimg, left, top, width, height, x, y, xscale, yscale,
                               color, alpha);
    return RValue_makeUndefined();
}
...
static RValue builtin_drawHealthbar(VMContext *ctx, RValue *args, MAYBE_UNUSED int32_t argCount) {
    ...
    if (showBack) {
        runner->renderer->vtable->drawRectangle(runner->renderer, x1, y1, x2, y2, backCol, runner->renderer->drawAlpha,
                                                false);
    }
...
static RValue builtin_drawTextColor(VMContext *ctx, RValue *args, MAYBE_UNUSED int32_t argCount) {
    ...
    PreprocessedText processedText = TextUtils_preprocessGmlTextIfNeeded(runner, str);
    runner->renderer->vtable->drawTextColor(runner->renderer, processedText.text, x, y, 1.0f, 1.0f, 0.0f, c1, c2, c3,
                                            c4, alpha);
    PreprocessedText_free(processedText);
    free(str);
    return RValue_makeUndefined();
...
static RValue builtin_drawTextColorTransformed(VMContext *ctx, RValue *args, MAYBE_UNUSED int32_t argCount) {
    ...
    PreprocessedText processedText = TextUtils_preprocessGmlTextIfNeeded(runner, str);
    runner->renderer->vtable->drawTextColor(runner->renderer, processedText.text, x, y, xscale, yscale, angle, c1, c2,
                                            c3, c4, alpha);
    PreprocessedText_free(processedText);
    free(str);
    return RValue_makeUndefined();
...
static RValue builtin_drawBackgroundStretched(VMContext *ctx, RValue *args, MAYBE_UNUSED int32_t argCount) {
    ...
    runner->renderer->vtable->drawSprite(runner->renderer, tpagIndex, x, y, 0.0f, 0.0f, xscale, yscale, 0.0f, 0xFFFFFF,
                                         runner->renderer->drawAlpha);
    return RValue_makeUndefined();
}
...
static RValue builtinDisplayGetGuiWidth(MAYBE_UNUSED VMContext *ctx, MAYBE_UNUSED RValue *args,
                                        MAYBE_UNUSED int32_t argCount) {
    Runner *runner = (Runner *) ctx->runner;
    return RValue_makeInt32(resolveGuiWidth(runner));
}

static RValue builtinDisplayGetGuiHeight(MAYBE_UNUSED VMContext *ctx, MAYBE_UNUSED RValue *args,
                                         MAYBE_UNUSED int32_t argCount) {
    Runner *runner = (Runner *) ctx->runner;
...
static RValue builtinPlaceMeeting(VMContext *ctx, RValue *args, int32_t argCount) {
    ...
    if (callerBBox.valid) {
        SpatialGridRange callerRange = SpatialGrid_computeCellRange(runner->spatialGrid, callerBBox.left,
                                                                    callerBBox.top, callerBBox.right,
                                                                    callerBBox.bottom);
        ...
        for (int32_t gx = callerRange.minGridX; callerRange.maxGridX >= gx && !found; gx++) {
            for (int32_t gy = callerRange.minGridY; callerRange.maxGridY >= gy && !found; gy++) {
                ...
                for (int32_t ci = 0; cellLen > ci; ci++) {
                    ...
                    if (filterByObject && !VM_isObjectOrDescendant(runner->dataWin, other->objectIndex, target))
                        continue;
        ...
        for (int32_t gx = callerRange.minGridX; callerRange.maxGridX >= gx && resultId == INSTANCE_NOONE; gx++) {
            for (int32_t gy = callerRange.minGridY; callerRange.maxGridY >= gy && resultId == INSTANCE_NOONE; gy++) {
                ...
                for (int32_t ci = 0; cellLen > ci; ci++) {
                    ...
                    if (filterByObject && !VM_isObjectOrDescendant(runner->dataWin, other->objectIndex, targetObjIndex))
                        continue;
...
static RValue builtinLayerGetId(VMContext *ctx, RValue *args, MAYBE_UNUSED int32_t argCount) {
    ...
    repeat(runtimeLayerCount, i) {
        ...
        if (runtimeLayer->dynamic && runtimeLayer->dynamicName != nullptr && strcmp(runtimeLayer->dynamicName, name) ==
            0) {
...
static RValue builtinLayerBackgroundExists(VMContext *ctx, RValue *args, MAYBE_UNUSED int32_t argCount) {
    ...
    repeat(count, i) {
        if ((int32_t) runtimeLayer->elements[i].id == elementId && runtimeLayer->elements[i].type ==
            RuntimeLayerElementType_Background) {
            return RValue_makeBool(true);
        }
    }
    return RValue_makeBool(false);
...
static RValue fontAddSpriteImpl(VMContext *ctx, int32_t spriteIndex, uint16_t *charCodes, uint32_t charCount,
                                bool proportional, int32_t sep) {
